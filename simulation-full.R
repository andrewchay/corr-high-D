...
alpha = 1
g = 1 / seq(1:P) ^ alpha * 3
X = mvrnorm(N, rep(0, n.beta0 + P), 
            Sigma = matrix(matrix_vec(n.beta0 + P), ncol = n.beta0 + P))
Z = X[, (n.beta0 + 1):(n.beta0 + P)]
X = as.vector(X[, 1:n.beta0])
...
num.nzeros = sum(g > sd(mu) / sqrt(sn) * sqrt(2 * log(P) / N) / 3)
...
for (i in 1:num.simu)
{
  e = rnorm(N, 0, abs(mu) / sqrt(sn))
  y = mu + e
  res0 = lm(y ~ X + Z[, 1:num.nzeros] - 1)
  res1 = partEst(Z, y, R1 = R11$R1, X = X, part1 = part1, R2 = R11$R2)
  ...
  Oracle = c(Oracle, res0$coef[1:n.beta0])
  parEst_O_cv_mcp = c(parEst_O_cv_mcp, res1$gammaOLS_cv_mcp)
  parEst_I_cv_mcp = c(parEst_I_cv_mcp, res1$gammaILS_cv_mcp)
  parEst_O_cv_lasso = c(parEst_O_cv_lasso, res1$gammaOLS_cv_lasso)
  parEst_I_cv_lasso = c(parEst_I_cv_lasso, res1$gammaILS_cv_lasso)
  parEst_O_bic_mcp = c(parEst_O_bic_mcp, res1$gammaOLS_bic_mcp)
  parEst_I_bic_mcp = c(parEst_I_bic_mcp, res1$gammaILS_bic_mcp)
  parEst_O_bic_lasso = c(parEst_O_bic_lasso, res1$gammaOLS_bic_lasso)
  parEst_I_bic_lasso = c(parEst_I_bic_lasso, res1$gammaILS_bic_lasso)
  sd_Oracle = c(sd_Oracle, sqrt(diag(vcov(res0))[1:n.beta0]))
  parEst_sd_cv_mcp = c(parEst_sd_cv_mcp, res1$sd_cv_mcp)
  parEst_sd_cv_lasso = c(parEst_sd_cv_lasso, res1$sd_cv_lasso)
  parEst_sd_bic_mcp = c(parEst_sd_bic_mcp, res1$sd_bic_mcp)
  parEst_sd_bic_lasso = c(parEst_sd_bic_lasso, res1$sd_bic_lasso)
  dlasso_cv_beta = c(dlasso_cv_beta, res2$beta_cv)
  dlasso_bic_beta = c(dlasso_bic_beta, res2$beta_bic)
  dlasso_cv_sd = c(dlasso_cv_sd, res2$sd_cv)
  dlasso_bic_sd = c(dlasso_bic_sd, res2$sd_bic)
  scaled_beta = c(scaled_beta, res3$beta)
  scaled_sd = c(scaled_sd, sqrt(res3$var))
  sigma_cv_mcp = c(sigma_cv_mcp, res1$sigma_cv_mcp)
  sigma_bic_mcp = c(sigma_bic_mcp, res1$sigma_bic_mcp)
  sigma_cv_lasso = c(sigma_cv_lasso, res1$sigma_cv_lasso)
  sigma_bic_lasso = c(sigma_bic_lasso, res1$sigma_bic_lasso)
  sigma_scaled = c(sigma_scaled, res3$sigma)
  sigma_dlasso_cv = c(sigma_dlasso_cv, res2$sigma_cv)
  sigma_dlasso_bic = c(sigma_dlasso_bic, res2$sigma_bic)
  sigma_Oracle = c(sigma_Oracle, sqrt(sum(res0$resid ^ 2) / res0$df.resid))
...
pmatrix1[1, 5] = round(sd(parEst_O_cv_mcp), 2)
pmatrix1[2, 5] = round(sd(parEst_O_bic_mcp), 2)
pmatrix1[3, 5] = round(sd(parEst_O_cv_lasso), 2)
pmatrix1[4, 5] = round(sd(parEst_O_cv_lasso), 2)
pmatrix1[5, 5] = round(sd(parEst_I_cv_mcp), 2)
pmatrix1[6, 5] = round(sd(parEst_I_bic_mcp), 2)
pmatrix1[7, 5] = round(sd(parEst_I_cv_lasso), 2)
pmatrix1[8, 5] = round(sd(parEst_I_cv_lasso), 2)
pmatrix1[9, 5] = round(sd(dlasso_cv_beta), 2)
pmatrix1[10, 5] = round(sd(dlasso_bic_beta), 2)
pmatrix1[11, 5] = round(sd(scaled_beta), 2)
pmatrix1[12, 5] = round(sd(Oracle), 2)

colnames(pmatrix1) = c("Bias", "SD", "MSE", 
                       "Coverage", "Simulation SD")
rownames(pmatrix1) = c("OLS CV MCP", "OLS BIC MCP", "OLS CV LASSO",
                       "OLS BIC LASSO", "ILS CV MCP", "ILS BIC MCP", 
                       "ILS CV LASSO", "ILS BIC LASSO", "DLASSO CV",
...
Oracle = data.frame(ratio = sigma_Oracle)
...
ErrorRatio = rbind(P_cv_mcp, P_bic_mcp, P_cv_lasso, P_bic_lasso, Oracle)
...
ErrorRatio = rbind(D_cv, D_bic, Scaled, Oracle)
ggplot(ErrorRatio, aes(ratio, fill = tag)) + 
  geom_density(alpha = 0.4) + labs(title = "SigmaHat / Sigma") + 
